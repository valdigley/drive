...
export async function downloadFile(url: string, filename: string, r2Key?: string): Promise<void> {
  try {
...
export async function downloadMultipleFiles(photos: { url: string; filename: string; r2Key?: string }[]): Promise<void> {
  for (const photo of photos) {
    await downloadFile(photo.url, photo.filename, photo.r2Key);
